...
def fake_retrieve_combined_knowledge(
    ...
    return type(
        ...
        (),
        {
            "runbooks": [
                (
                    "Database connection pool exhaustion causes "
                    "connection timeouts and request failures."
                )
            ],
            "similar_incidents": [
                (
                    "Previous incident had JDBC connection "
                    "pool exhaustion."
                )
            ],
        },
...
def test_complete_agent_investigation(monkeypatch):
    ...
    assert "understand_context" in result["completed_goals"]
    assert "check_service_health" in result["completed_goals"]
    assert "collect_error_evidence" in result["completed_goals"]
    ...
    assert result["similar_incidents"] == [
        "Previous incident had JDBC connection pool exhaustion."
    ]
